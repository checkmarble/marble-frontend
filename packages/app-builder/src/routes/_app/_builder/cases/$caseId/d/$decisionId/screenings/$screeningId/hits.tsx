...
export const Route = createFileRoute('/_app/_builder/cases/$caseId/d/$decisionId/screenings/$screeningId/hits')({
  component: CaseSanctionsHitsPage,
});

function CaseSanctionsHitsPage() {
  const { t } = useTranslation(casesI18n);
  const { caseDetail, screening, decision, dataModel, pivots } = useLoaderData({
      ...
      <div className="col-span-full flex flex-row gap-12 p-4">
        <ScreeningReviewSection
          ...
          onRefineSuccess={(screeningId) => {
            navigate(
              `/cases/${fromUUIDtoSUUID(caseDetail.id)}/d/${fromUUIDtoSUUID(decision.id)}/screenings/${fromUUIDtoSUUID(screeningId)}`,
            );
          }}
        />
