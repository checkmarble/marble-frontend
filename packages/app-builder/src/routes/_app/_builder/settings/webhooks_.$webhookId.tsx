...
export const Route = createFileRoute('/_app/_builder/settings/webhooks_/$webhookId')({
  staticData: {
    hideTabs: true,
    BreadCrumbs: [
      ({ isLast: _ }: BreadCrumbProps) => {
        const { t } = useTranslation(['settings']);
        return <span className="text-s font-bold">{t('settings:webhook_details')}</span>;
      },
    ],
  },
  loader: ({ params }) => webhookDetailLoader({ data: { params } }),
  component: WebhookDetail,
});

function WebhookDetail() {
  const { t } = useTranslation(['settings']);
  const { webhook, isEditWebhookAvailable, isDeleteWebhookAvailable, webhookStatus } = Route.useLoaderData();
