...
import { updateDataDisplayFn } from '@app-builder/server-fns/settings';
...
const dataDisplayLoader = createServerFn()
  .middleware([authMiddleware])
  .handler(async function dataDisplayLoader({ context }) {
...
export const Route = createFileRoute('/_app/_builder/settings/data-display')({
  loader: () => dataDisplayLoader(),
  component: DataDisplaySettings,
});

function DataDisplaySettings() {
  const { t } = useTranslation(['common', 'settings', 'data']);
  const { dataModelWithTableOptions } = Route.useLoaderData();
  const exportOrgMutation = useExportOrgMutation();

  const updateDataDisplay = useServerFn(updateDataDisplayFn);
  const updateMutation = useMutation({
    mutationFn: (value: Record<string, SetDataModelTableOptionsBody>) => updateDataDisplay({ data: value }),
  });
