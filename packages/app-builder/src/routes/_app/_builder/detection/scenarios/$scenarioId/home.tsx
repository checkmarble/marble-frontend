...
const homeLoader = createServerFn()
  ...
  .handler(async function homeLoader({ data, context }) {
    ...
    const [scheduledExecutions, testRuns, liveIteration] = await Promise.all([
      ...
      testRunRepository.listTestRuns({ scenarioId }),
...
export const Route = createFileRoute('/_app/_builder/detection/scenarios/$scenarioId/home')({
  loader: ({ params }) => homeLoader({ data: { params } }),
  component: ScenarioHome,
});

function ScenarioHome() {
  const { t } = useTranslation(['common', 'scenarios']);
  const { featureAccess, scheduledExecutions, liveIterationSchedule } = Route.useLoaderData();
          ...
          {currentScenario.description || featureAccess.isEditScenarioAvailable ? (
            <Page.Description withIcon={false} className="flex-1">
              <ScenarioDescriptionEditable
                isEditScenarioAvailable={featureAccess.isEditScenarioAvailable}
...
function TestRunSection({ scenarioId, access }: { scenarioId: string; access: FeatureAccessLevelDto }) {
  ...
  const { testRuns } = Route.useLoaderData();
