  ...
  staticData: {
    BreadCrumbs: [
      ({ isLast }: BreadCrumbProps) => {
        const { customList } = Route.useLoaderData();

        return (
          <BreadCrumbLink
            to="/detection/lists/$listId"
            params={{ listId: fromUUIDtoSUUID(customList.id) }}
            isLast={isLast}
          >
            {customList.name}
          </BreadCrumbLink>
    ...
    >
      ...
      >
        <Modal.Content onClick={(e) => e.stopPropagation()}>
          ...
          <Modal.Footer>
            <Modal.FooterButton isCloseButton variant="primary" label={t('common:understand')} leadingIcon="tick" />
          </Modal.Footer>
        </Modal.Content>
      </Modal.Root>
    </UploadAsCsvDropzone>
