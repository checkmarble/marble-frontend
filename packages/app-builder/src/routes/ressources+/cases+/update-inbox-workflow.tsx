...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const { success, data, error } = updateInboxWorkflowPayloadSchema.safeParse(raw);
  ...
  await inboxApi.updateInbox(data.inboxId, {
    name: inbox.name,
    caseReviewManual: data.caseReviewManual,
    caseReviewOnCaseCreated: data.caseReviewOnCaseCreated,
    caseReviewOnEscalate: data.caseReviewOnEscalate,
  });
