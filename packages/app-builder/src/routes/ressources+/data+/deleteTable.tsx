...
export const action = createServerFn([authMiddleware], async function deleteTableAction({ request, context }) {
  ...
  const { success, error, data } = deleteTablePayloadSchema.safeParse(raw);

  if (!success) return { success: false as const, errors: z.treeifyError(error) };

  try {
    const result = await apiClient.deleteDataModelTable(data.tableId, {
    ...
    console.error('deleteDataModelTable error:', error);
