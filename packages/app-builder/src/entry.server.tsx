  ...
  return new Promise((resolve, reject) => {
    ...
    const { pipe, abort } = renderToPipeableStream(App, {
      ...
      onShellReady() {
        ...
        const firebaseUrl =
          (clientEnv.FIREBASE_CONFIG.withEmulator ?? appConfig.auth.firebase.isEmulator)
            ? (clientEnv.FIREBASE_CONFIG.authEmulatorUrl ?? appConfig.auth.firebase.emulatorUrl)
        ...
        const fbAuthDomain =
          clientEnv.FIREBASE_CONFIG.options.authDomain ?? appConfig.auth.firebase.authDomain;
