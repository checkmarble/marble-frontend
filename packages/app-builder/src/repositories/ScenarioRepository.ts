...
import { adaptScenarioIterationDto, adaptScenarioIterationRuleDto, type ScenarioIteration } from '@app-builder/models/scenario';
...
export function getScenarioRepository() {
  return (marbleApiClient: MarbleApi) => ({
    ...
    getScenarioIteration: async ({ iterationId }: { iterationId: string }): Promise<ScenarioIteration> => {
