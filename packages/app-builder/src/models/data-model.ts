import { type DataModel as DataModelDto, type DataModelField as DataModelFieldDto, type LinkToSingle as LinkToSingleDto } from '@marble-api';
...
function adaptFieldDto(dataModelFieldsDto: {[key: string]: DataModelFieldDto}): DataModelField[] {
...
function adaptLinkToSingleDto(linksToSingleDto: {[key: string]: LinkToSingleDto}): LinksToSingle[] {
...
export function adaptDataModelDto(dataModelDto: DataModelDto): DataModel[] {
  ...
  Object.keys(dataModelDto.tables).forEach((key) => {
    ...
    if (dataModelDto.tables[key].links_to_single !== undefined) {
      linksToSingle = adaptLinkToSingleDto(dataModelDto.tables[key].links_to_single)
    }
    dataModel.push({
