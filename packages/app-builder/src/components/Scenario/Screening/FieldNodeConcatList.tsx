...
function sameConcatContents(left: StringConcatAstNode[], right: StringConcatAstNode[]): boolean {
  return (
    left.length === right.length &&
    left.every(
      (concat, index) =>
        concat.id === right[index]?.id &&
        concat.children.length === right[index]?.children.length &&
        concat.children.every((child, childIndex) => child.id === right[index]?.children[childIndex]?.id),
    )
  );
}
    ...
    <DragDropContext onDragEnd={onDragEnd} autoScrollerOptions={{ disabled: true }}>
      <div onBlur={onBlur} className="flex flex-col gap-sm">
        {concats.length === 0 && !viewOnly ? (
          <Button mode="icon" variant="secondary" appearance="link" onClick={() => applyConcats([newEmptyConcat()])}>
              ...
              {!viewOnly ? (
                <div className="flex flex-row">
                  <Button
                    ...
                    variant="secondary"
                    appearance="link"
                    onClick={() => applyConcats(splice(concats, concatIndex, 1, []))}
                  >
