...
export const ClearAllFiltersLink = function ClearAllFiltersButton({
  ref,
  ...props
}: Omit<RemixLinkProps, 'className' | 'ref'> & { ref?: React.Ref<HTMLAnchorElement> }) {
...
export const ClearAllFiltersButton = function ClearAllFiltersButton({
  ref,
  ...props
}: Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, 'className' | 'ref'> & { ref?: React.Ref<HTMLButtonElement> }) {
  const { t } = useTranslation(filtersI18n);
  return (
    <button
      className={CtaV2ClassName({ variant: 'secondary', color: 'grey', className: 'shrink-0' })}
