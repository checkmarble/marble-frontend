...
function UpdateJobsContent({ data }: UpdateJobsContentProps) {
  ...
  return (
    <div className="col-span-full flex flex-col gap-sm">
      ...
      <GridContent data={data} withPanel />
    </div>
  );
}

function GridContent({ data, withPanel }: { data: ContinuousScreeningUpdateJobSummary[]; withPanel?: boolean }) {
  const { t } = useTranslation(['continuousScreening']);
