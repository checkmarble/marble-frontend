...
export function createRightPanel(name: string) {
  ...
  const RightPanelContent = forwardRef<HTMLDivElement, Dialog.DialogContentProps>(({ className, ...props }, ref) => {
    return (
      <Dialog.Content
        ...
        {...props}
        className={clsx(
          'bg-surface-card absolute inset-y-0 end-0 size-full gap-md overflow-y-scroll p-md pe-[calc(1rem-var(--scrollbar-width))] shadow-sm lg:gap-lg lg:p-lg lg:pe-[calc(1.5rem-var(--scrollbar-width))]',
  ...
  function RightPanelTitle(props: Dialog.DialogTitleProps) {
    return (
      <Dialog.Title
        {...props}
        className={clsx('text-grey-primary text-l flex flex-row items-center gap-sm font-bold', props.className)}
      />
    );
  }
