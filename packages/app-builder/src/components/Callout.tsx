...
const callout = cva('text-s text-grey-00 flex flex-row items-center gap-2 rounded p-2 font-normal', {
  variants: {
    /**
...
export function Callout({ children, className, color = 'purple', variant = 'soft', bordered, ...otherProps }: CalloutProps) {
...
export function CalloutV2({ children, className, ...otherProps }: React.ComponentPropsWithoutRef<'aside'>) {
  ...
  return (
    <aside className={clsx('bg-purple-98 text-s text-purple-65 flex flex-row gap-2 rounded-lg p-4 font-normal', className)} {...otherProps}>
