...
export function useLaidOutGraph({ data, dataModel }: { data: GraphData; dataModel: DataModel }) {
  ...
  const { clusterThreshold, showHypernodes } = useGraphViewSettings();
  ...
  const filteredLayout = useMemo(() => {
    ...
    return layoutGraph(clustered.nodes, clustered.edges, flatGraph.startKey);
  ...
  const autoLayoutElements = useCallback(
    (layoutNodes: GraphRfNode[], layoutEdges: GraphRfEdge[]) =>
      layoutGraph(layoutNodes, layoutEdges, resolveStartKey(layoutNodes, layoutNodes[0]?.id ?? '')),
    [],
  );
