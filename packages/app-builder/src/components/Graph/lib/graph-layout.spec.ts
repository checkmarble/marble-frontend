...
import { clusterGraphElements, layoutGraph } from './graph-layout';
...
describe('clusterGraphElements', () => {
  ...
  it('chips a branch headed by a non-person record', () => {
    const { nodes, edges } = fanFixture(9);
    for (const node of nodes) {
      if (node.type !== 'person') continue;
      node.data.semanticType = 'account';
      node.data.subEntity = 'generic';
      node.data.objectType = 'accounts';
    }

    const chips = chipsOf(clusterGraphElements(nodes, edges, options(9)).nodes);
    expect(chips).toHaveLength(1);
    expect(chips[0]!.data.root.semanticType).toBe('account');
    expect(chips[0]!.data.root.objectId).toBe('root');
  });
