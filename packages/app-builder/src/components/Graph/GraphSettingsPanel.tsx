...
import { type PersonListItem, personListItemKey, type StructuralPersonListItem } from './lib/connected-person-list';
import { graphI18n } from './lib/graph-i18n';
import { type GraphObjectRef, nodeKey, rootNodeId } from './lib/graph-keys';
...
function PersonRow({ item, showTags }: { item: StructuralPersonListItem; showTags: boolean }) {
  const { t } = useTranslation(graphI18n);
  const setSelectedObject = useSetSelectedObject();
  const { hoverNode } = useGraphInteractionActions();
  const selectionMode = useSelectionMode();
  const checkableId = rootNodeId(item.id);
  const isNodeChecked = useIsNodeChecked(checkableId);
  const { toggleCheckedNode } = useGraphInteractionActions();

  const title = resolveTitle(item.ref.label, item.ref.objectId);
  ...
  function handleClick() {
    ...
    if (item.type === 'cluster') {
      setSelectedObject({
        ...
        internalEdgeCount: item.internalEdgeCount,
        persons: item.members,
      });
      return;
    }
    setSelectedObject({ ...item.ref, nodeType: 'person', persons: [item] });
  }
  ...
  return (
    <li
      ...
      onMouseEnter={() => {
        if (!selectionMode) hoverNode(item.id);
      }}
      onMouseLeave={() => hoverNode(null)}
      onClick={handleClick}
    >
      <div className="flex flex-col gap-xs">
        <label className="flex flex-wrap items-center gap-sm" htmlFor={checkableId}>
          {selectionMode && (
            <Checkbox
              size="small"
          ...
          <Icon icon={subEntityIcon(item.ref)} className="size-4 shrink-0 text-purple-primary" />
          <span className="text-sm">{title}</span>
          {item.type === 'cluster' ? (
            <span className="text-grey-secondary text-xs">{t('graph:count.nodes', { count: item.nodeCount })}</span>
          ) : null}
          <Tag color="grey" size="small">
            {item.ref.objectType}
          </Tag>
          <ObjectRiskBadge {...item.ref} />
        </label>
        {showTags && item.type === 'person' ? <ObjectTags {...item.ref} /> : null}
      </div>
    </li>
  );
}
...
function PersonListDetail({
  ...
  const structural = persons.filter((item): item is StructuralPersonListItem => item.type !== 'pivot');
  const total = structural.length;
  const hasMore = total > PERSON_LIST_PREVIEW_COUNT;
  const displayedPersons = showAll || !hasMore ? structural : structural.slice(0, PERSON_LIST_PREVIEW_COUNT);
      ...
      {total === 0 ? (
        ...
        <>
          <ul className="space-y-sm ps-md">
            {displayedPersons.map((item) => (
              <PersonRow key={personListItemKey(item)} item={item} showTags={showTags} />
            ))}
          </ul>
          {hasMore ? (
