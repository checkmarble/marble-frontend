...
import {
  ...
  pivotRefFromRfNode,
  samePersonListItems,
} from './lib/connected-person-list';
import { graphFitViewOptions } from './lib/graph-fit-view';
import { graphI18n } from './lib/graph-i18n';
import { nodeKey } from './lib/graph-keys';
import { type GraphRfEdge, type GraphRfNode } from './lib/graph-rf-types';
...
export function GraphImpl({ data, dataModel }: GraphImplProps) {
  ...
  const connectedPersonsForNode = useCallback(
    (nodeId: string) => connectedPersonListItems(nodes, graphIndex.neighbors.get(nodeId), flatGraph.nodes),
    [graphIndex, nodes, flatGraph.nodes],
  );
  ...
  useEffect(() => {
    ...
    if (samePersonListItems(selectedObject.persons, persons)) return;
    ...
    (_event, node) => {
      ...
      if (node.type === 'cluster') {
        setSelectedObject({
          ...
          persons: personListItemsFromMemberIds(node.data.memberIds, flatGraph.nodes),
        });
        return;
      }
