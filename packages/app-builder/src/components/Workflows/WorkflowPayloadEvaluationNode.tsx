...
import { getErrorsForNode } from '../AstBuilder/edition/helpers';
import { AstBuilderNodeSharpFactory } from '../AstBuilder/edition/node-store';
import { OperatorSelect, type OperatorSelectOptions } from '../AstBuilder/edition/OperatorSelect';
...
// Filtered operators for workflow payload evaluation
// Include basic comparison, string matching, and list operations
const workflowOperatorFunctionsOptions: OperatorSelectOptions<string> = {
  '=': {},
  '≠': { keywords: ['!='] },
  '<': {},
  '<=': {},
...
export const WorkflowPayloadEvaluationNode = memo(function WorkflowPayloadEvaluationNode(props: {
  root?: boolean;
  path: string;
}) {
  ...
  const children = match(node.value)
    .when(isMainAstBinaryNode, (node) => {
      ...
      const children = (
        <>
          <WorkflowPayloadEvaluationNode path={`${props.path}.children.0`} />
          <OperatorSelect
            hideArrow
            options={workflowOperatorFunctionsOptions}
            validationStatus={hasDirectError ? 'error' : 'valid'}
            operator={node.name}
            onOperatorChange={setOperator}
          />
          <WorkflowPayloadEvaluationNode path={`${props.path}.children.1`} />
        </>
      );
    ...
    .when(isKnownOperandAstNode, (node) => {
      ...
      return (
        <WorkflowPayloadEvaluationOperand
          node={node}
          onChange={setNode}
...
WorkflowPayloadEvaluationNode.displayName = 'WorkflowPayloadEvaluationNode';
...
// Custom operand component for workflow payload evaluation
// This filters the available options to include Fields and specific Functions, excluding Modeling
function WorkflowPayloadEvaluationOperand({
  node,
  onChange,
  enumValues,
  validationStatus,
  ...
  return (
    <EditionAstBuilderOperand
      ...
      optionsDataType={(option) => {
        // Remove modeling options (nesting/parentheses)
        if (option.operandType === 'Modeling') {
          return false;
        }

        // Filter functions to only include date functions and string matching functions
        if (option.operandType === 'Function') {
          const astNode = option.astNode;
          // Allow date functions and string matching functions
          if (
            astNode.name === 'TimeAdd' ||
            astNode.name === 'TimestampExtract' ||
            astNode.name === 'TimeNow' ||
            astNode.name === 'FuzzyMatch'
          ) {
            return true;
          }
          return false;
        ...
        // Allow fields, constants, enums, and other operand types except modeling
        return true;
      }}
    />
  );
