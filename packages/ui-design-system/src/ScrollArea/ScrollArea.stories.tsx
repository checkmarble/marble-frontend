...
export const V1: StoryFn = () => (
  <ScrollArea.Root className="border-grey-50 w-fit rounded-sm border shadow-md">
        ...
        {Array.from({ length: 15 }).map((_, index) => (
          <li
            className={clsx(
              ...
              index % 2 === 0 ? 'bg-grey-90' : 'bg-surface-card',
            )}
            key={index}
          >
            <span>{index}</span>
...
export const V2: StoryFn = () => (
  <ScrollAreaV2 orientation="both" className="border-grey-50 max-h-72 w-fit max-w-[100px] rounded-sm border shadow-md">
    <ul>
      {Array.from({ length: 15 }).map((_, index) => (
        <li
          className={clsx('flex w-48 flex-col px-3 py-2 shadow-xs', index % 2 === 0 ? 'bg-grey-90' : 'bg-surface-card')}
          key={index}
        >
          <span>{index}</span>
        </li>
