...
export const MenuPopover = React.forwardRef<HTMLDivElement, MenuProps>(function MenuPopover(props, ref) {
  return (
    <Ariakit.Menu
      ...
      {...props}
      className={clsx(
        'bg-surface-card border-grey-90 flex max-h-[min(var(--popover-available-height),500px)] -translate-y-1 overflow-hidden rounded-sm border opacity-0 shadow-md outline-hidden transition-all data-enter:translate-y-0 data-enter:opacity-100',
        props.className,
      )}
    />
  );
