...
function TableTH<TData extends RowData, TValue>({
  ...
  return (
    <th
      ...
      className={clsx(
        'border-grey-border bg-surface-card relative h-12 w-full border-b border-r px-sm last:border-r-0',
        {
  ...
  return (
    ...
    >
      {row.getVisibleCells().map((cell, index) => {
        ...
        return (
          <td key={`${cell.id}-${index}`} className="text-s w-full truncate px-sm font-normal">
